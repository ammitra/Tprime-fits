...
def _load_CR_rpf(poly_order):
    twoD_CRonly = TwoDAlphabet('ttbarCR','LP.json', loadPrevious=True)
...
def test_make():
    twoD = TwoDAlphabet('THfits_LP_CR','LP.json',loadPrevious=False)
...
def test_fit(signal, tf='', defMinStrat=0, extra='--robustHesse 1'):
    ...
    twoD.MLfit('TprimeB-{}-{}_area'.format(signal,tf),rMin=-1,rMax=5,verbosity=2,defMinStrat=defMinStrat,extra=extra)

def test_plot(signal, tf='', prefit=False, regionsToGroup=[], dataOnly=False):
    working_area = 'THfits_LP_CR'
    twoD = TwoDAlphabet(working_area, '{}/runConfig.json'.format(working_area), loadPrevious=True)
    subset = twoD.ledger.select(_select_signal, 'TprimeB-{}'.format(signal), tf)
    twoD.StdPlots('TprimeB-{}-{}_area'.format(signal,tf), subset, prefit=prefit, regionsToGroup=regionsToGroup,dataOnly=dataOnly)
...
def test_GoF(SRorCR, signal, tf='', condor=True):
    #assert SRorCR == 'CR'
    working_area = 'THfits_LP_{}'.format(SRorCR)
    twoD = TwoDAlphabet(working_area, '{}/runConfig.json'.format(working_area), loadPrevious=True)
...
def test_GoF_plot(SRorCR, signal, tf='', condor=True):
    ...
    signame = 'TprimeB-'+signal
    plot.plot_gof('THfits_LP_'+SRorCR,'{}-{}_area'.format(signame,tf), condor=condor)
...
if __name__ == "__main__":
	...
	#test_fit('1800-125', tf=opt_name, defMinStrat=1, extra='--robustFit 1')

...
def _load_CR_rpf(poly_order):
    twoD_CRonly = TwoDAlphabet('ttbarCR','SR.json', loadPrevious=True)
...
def test_make():
    twoD = TwoDAlphabet('THfits_LP_SR','SR.json',loadPrevious=False)
...
if __name__ == "__main__":
	...
	#if opt_name in ['1x0','2x1']: continue
	test_fit('1800-125', tf=opt_name, defMinStrat=1, extra='--robustFit 1')
	#test_plot('1800-125', tf=opt_name)
	#test_GoF('SR','1800-125',tf=opt_name,condor=True)
	#test_GoF_plot('SR', '1800-125', tf=opt_name, condor=True)	

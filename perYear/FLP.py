...
def _load_CR_rpf(poly_order):
    twoD_CRonly = TwoDAlphabet('ttbarCR','FLP.json', loadPrevious=True)
...
def test_make():
    twoD = TwoDAlphabet('THfits_FLP_CR','FLP.json',loadPrevious=False)
    ...
    # qcd_l = qcd_f*rpfL
    qcd_l = qcd_f.Multiply(fail_name.replace('fail','loose'),qcd_rpfL)
    # qcd_p = qcd_l*rpfT
    qcd_p = qcd_l.Multiply(fail_name.replace('fail','pass'),qcd_rpfT)

    twoD.AddAlphaObj('Background',l,qcd_l,title='Background')
    twoD.AddAlphaObj('Background',p,qcd_p,title='Background')
    
    twoD.Save()

def test_fit(signal, tf='', defMinStrat=0, extra='--robustHesse 1'):
    working_area = 'THfits_FLP_CR'
    ...
    twoD.MLfit('TprimeB-{}-{}_area'.format(signal,tf),rMin=-1,rMax=20,verbosity=2,defMinStrat=defMinStrat,extra=extra)
...
def test_GoF(SRorCR, signal, tf='', condor=True):
    #assert SRorCR == 'CR'
    working_area = 'THfits_FLP_{}'.format(SRorCR)
    twoD = TwoDAlphabet(working_area, '{}/runConfig.json'.format(working_area), loadPrevious=True)
...
def test_GoF_plot(SRorCR, signal, tf='', condor=True):
    ...
    signame = 'TprimeB-'+signal
    plot.plot_gof('THfits_FLP_'+SRorCR,'{}-{}_area'.format(signame,tf), condor=condor)
...
if __name__ == "__main__":
    #test_make()
    #test_fit('1800-125', defMinStrat=0, extra='--robustHesse 1')
    #test_plot('1800-125')
    #test_GoF('CR', '1800-125', tf='', condor=True)
    test_GoF_plot('CR', '1800-125', tf='', condor=True)

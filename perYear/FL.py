...
def _load_CR_rpf(poly_order):
    twoD_CRonly = TwoDAlphabet('ttbarCR','FL.json', loadPrevious=True)
...
def test_make():
    twoD = TwoDAlphabet('THfits_FL_CR','FL.json',loadPrevious=False)
...
def test_GoF(SRorCR, signal, tf='', condor=True):
    #assert SRorCR == 'CR'
    working_area = 'THfits_FL_{}'.format(SRorCR)
    twoD = TwoDAlphabet(working_area, '{}/runConfig.json'.format(working_area), loadPrevious=True)
...
def test_GoF_plot(SRorCR, signal, tf='', condor=True):
    ...
    signame = 'TprimeB-'+signal
    plot.plot_gof('THfits_FL_'+SRorCR,'{}-{}_area'.format(signame,tf), condor=condor)
